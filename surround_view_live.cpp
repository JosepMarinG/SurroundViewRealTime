...
int main(int argc, char** argv)
{
    ...
    while (true) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (origin_dir_img[i].empty()) {
                std::cerr << "Empty frame from camera " << i << std::endl;
                any_empty = true;
                break;
            }
        ...
        if (any_empty) {
            continue; // Salir si alguna cámara no proporciona un frame válido
        }
    ...
    #endif
    // Mostrar
        cv::imshow("Surround View", out_put_img);
        if (cv::waitKey(1) == 'q') break;
    }

    for (int i = 0; i < 4; ++i) cams[i].release();
